...
for i in range(2, check_row + 2):
    ...
    for j in range(7):
        if res7[j] == None:
            ...
            if (re7[j]) == None:
                re_lin += 1
                # print('re_lin',re_lin)
            elif (re7[j])['scale']['pv']['oneSelf'] == '0':
                re_lin += 1
                # print((re7[j])['scale']['pv']['oneSelf'])
                # print('re_lin1',re_lin)
    ...
    elif 4 <= res_lin < 7 and re_lin < 3:
        print('\n本周有超过4天为0的数据，本周', + res_lin, '天数据为0，上周', + re_lin, '天数据为0,数据店铺ID为', str(sheet['A' + str(i)].value))

print("\n判断前700条数据中有没有超过350的0或空")
for s in range(7):
    if sum_0andnone[s] >= 350:
        print(column_name[s] + '列前700条数据中有超过350的0或空')
    print(sum_0andnone[s])
