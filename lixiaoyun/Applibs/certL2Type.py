...
con = r'C:\Users\admin\Desktop\1213.txt'
c = r'C:\Users\admin\Desktop\配置文件.txt'
config = open(c,'r+')
b = config.read()
...
def shop_file(sum_text,exe_text):
    ...
    for vlue in range(sum_text):
        ...
        for i in range(exe_text_sub_sum_list):
            ...
            sum += 1
    wb.save(r"C:/Users/admin/Desktop/涵盖证书类型.xlsx")
    print("保存成功")
if __name__ == "__main__":
    print (check_json_format("""{"a":1}"""))
    print (check_json_format("""{'a':1}"""))
    print (check_json_format({'a': 1}))
    list = list(check_json_format(b))
    list_len = len(check_json_format(b))
    sum_text = len(check_json_format(b)['certType'])
    exe_text = check_json_format(b)['certType']
